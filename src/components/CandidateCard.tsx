...
export default function CandidateCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-5 py-4 border-b border-gray-100 transition-all duration-150 group flex items-center gap-4 ${
        isActive
    ...
    >
      ...
      <div className="relative flex-shrink-0">
        <img
          ...
          alt={candidate.login}
          className={`w-10 h-10 rounded-full object-cover ${isActive ? "ring-2 ring-primary-400 ring-offset-1" : ""}`}
        />
        {candidate.shortlisted && (
          <div className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-primary-500 rounded-full flex items-center justify-center ring-2 ring-white">
      ...
      {/* Arrow */}
      <svg className={`w-4 h-4 flex-shrink-0 transition-colors ${isActive ? "text-primary-500" : "text-gray-300 group-hover:text-gray-400"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
